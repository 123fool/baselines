"""
Priority 4: BTR + PALM + TEL — Evaluation Script.

Evaluates using PALM+TEL-enhanced sampling pipeline.
Loads combined checkpoint {controlnet, palm, tel} and uses
sample_using_controlnet_and_z_palm_tel for inference.
"""
...
from sampling_palm_tel import sample_using_controlnet_and_z_palm_tel
...
def evaluate_pair_palm_tel(autoencoder, diffusion, controlnet, palm, tel,
                           row, scale_factor, load_latent, las_m=1):
    """Evaluate one pair using PALM+TEL-enhanced sampling."""
    ...
    predicted = sample_using_controlnet_and_z_palm_tel(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Evaluate Priority 4 BTR+PALM+TEL ControlNet')
    ...
    parser.add_argument('--cnet_ckpt',   required=True, type=str,
                        help='Combined checkpoint with controlnet/palm/tel')
    parser.add_argument('--output_dir',  required=True, type=str)
    parser.add_argument('--max_pairs',   default=50,    type=int)
    parser.add_argument('--model_name',  default='btc_palm_tel', type=str)
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok=True)

    print(f"[Priority 4] BTR + PALM + TEL Evaluation")
    print(f"  CSV: {args.dataset_csv}")
    print(f"  Checkpoint: {args.cnet_ckpt}")
    ...
    palm = PALM(context_dim=8, latent_channels=3).to(DEVICE)
    tel = TEL(d_model=64).to(DEVICE)
    ...
    if isinstance(ckpt, dict) and 'controlnet' in ckpt:
        ...
        print(f"  Loaded PALM+TEL checkpoint")
    else:
        raise ValueError("Checkpoint must be dict with 'controlnet', 'palm', 'tel' keys")
    ...
    for idx, row in tqdm(eval_pairs.iterrows(), total=len(eval_pairs),
        ...
        try:
            res = evaluate_pair_palm_tel(
                autoencoder, diffusion, controlnet, palm, tel,
    ...
    summary = {
        ...
        'method': 'priority_4_btc_palm_tel',
